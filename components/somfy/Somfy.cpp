
#include "esphome/components/cover/cover.h"
#include "esphome/components/button/button.h"
#include "esphome/components/cc1101/cc1101.h"
#include "esphome/core/component.h"
#include "esphome/components/remote_transmitter/remote_transmitter.h"
#include "EsphomeRollingCodeStorage.h"
#include "Somfy.h"

namespace esphome {
namespace somfy {

void SomfyComponent::setup() {
    storage_ = new EsphomeRollingCodeStorage(remote_address_);
    // Attach the prog button
    if (this->cover_prog_button_) {
        this->cover_prog_button_->add_on_press_callback(
            [=, this] { this->program(); }
        );
    }
}

void SomfyComponent::dump_config() {
    ESP_LOGCONFIG(TAG,
                    "  Remote address: 0x%" PRIx32 "\n"
                    "  Repeat command: %dx\n",
                    this->remote_address_, this->repeat_);
}

void SomfyComponent::program() {
    ESP_LOGI(TAG, "PROG");
    send_command_internal(Command::Prog);
}

void SomfyComponent::send_command_internal(Command command) {
    this->transmit_trigger_.trigger();
    send_command(command);
    this->complete_trigger_.trigger();
}

void SomfyComponent::send_command(Command command) {
    const uint16_t rollingCode = this->storage_->nextCode();
    ESP_LOGD(TAG, "Sending %dx command: 0x%x for button addr: 0x%" PRIx32 " with rolling code: 0x%04X", this->repeat_, static_cast<unsigned int>(command), this->remote_address_, rollingCode);
    uint8_t frame[7];
    build_frame(frame, command, rollingCode);
    remote_base::RawTimings t;
    build_timings(t, frame, 2);
    for (int i = 0; i < this->repeat_; i++) {
        build_timings(t, frame, 7);
    }
    auto call = this->remote_transmitter_->transmit();
    call.get_data()->set_data(t);
    call.perform();
}

void SomfyComponent::build_frame(uint8_t *frame, Command command, uint16_t code) {
    const uint8_t button = static_cast<uint8_t>(command);
    frame[0] = 0xA7;          // Encryption key. Doesn't matter much
    frame[1] = button << 4;   // Which button did  you press? The 4 LSB will be the checksum
    frame[2] = code >> 8;     // Rolling code (big endian)
    frame[3] = code;          // Rolling code

    frame[4] = this->remote_address_ >> 16;  // Remote address
    frame[5] = this->remote_address_ >> 8;   // Remote address
    frame[6] = this->remote_address_;        // Remote address

    // Checksum calculation: a XOR of all the nibbles
    uint8_t checksum = 0;
    for (uint8_t i = 0; i < 7; i++) {
        checksum = checksum ^ frame[i] ^ (frame[i] >> 4);
    }
    checksum &= 0b1111;  // We keep the last 4 bits only

    // Checksum integration
    frame[1] |= checksum;

    // Obfuscation: a XOR of all the bytes
    for (uint8_t i = 1; i < 7; i++) {
        frame[i] ^= frame[i - 1];
    }
}

void SomfyComponent::build_timings(remote_base::RawTimings & t, uint8_t *frame, uint8_t sync) {
    constexpr int32_t SYMBOL = 640;

    if (sync == 2) {  // Only with the first frame.
        // Wake-up pulse & Silence
        send_high(t, 9415);
        send_low(t, 9565 + 80000); // was delay(80)
    }

    // Hardware sync: two sync for the first frame, seven for the following ones.
    for (int i = 0; i < sync; i++) {
        send_high(t, 4 * SYMBOL);
        send_low(t, 4 * SYMBOL);
    }

    // Software sync
    send_high(t, 4550);
    send_low(t, SYMBOL);

    // Data: bits are sent one by one, starting with the MSB.
    for (uint8_t i = 0; i < 56; i++) {
        if (((frame[i / 8] >> (7 - (i % 8))) & 1) == 1) {
        send_low(t, SYMBOL);
        send_high(t, SYMBOL);
        } else {
        send_high(t, SYMBOL);
        send_low(t, SYMBOL);
        }
    }

    // Inter-frame silence
    send_low(t, 415 + 30000); // was delay(30)
}

void SomfyComponent::send_high(remote_base::RawTimings & t, int32_t durationUsecs) {
    t.push_back(static_cast<int32_t>(durationUsecs));
}

void SomfyComponent::send_low(remote_base::RawTimings & t, int32_t durationUsecs) {
    t.push_back(-static_cast<int32_t>(durationUsecs));
}

} // namespace somfy
} // namespace esphome
